...
MIN_LANE_SIGNAL_QUALITY_RATIO = 1.5  # max(hist) < MIN_LANE_SIGNAL * 1.5 ise red et
...
WHITE_HSV_LOW_NORMAL = (0,   0,  120)
WHITE_HSV_HIGH_NORMAL = (180, 85, 255)
...
# ---------------------------------------------------------------------------
KP = 0.60   # oransal kazanç — daha responsive
KD = 0.44   # türevsel kazanç — virajlarda güçlü

# Büyük hatalar için dinamik kazanç artışı
KP_LARGE_ERROR_MULT = 1.3  # |err| > 30 iken KP'yi çarp
KD_LARGE_ERROR_MULT = 1.5  # |err| > 30 iken KD'yi çarp

# Derivative cap (salınım önleme)
DERIV_CAP = 600

# Crossing (viraj) için KD artırma (sollama, dönüş vb.)
CROSSING_KD_MULT = 1.8  # |derivative| > 50 iken KD'yi çarp
...
K_SPEED    = 0.28 # hız = BASE - K_SPEED * |hata|
...
# Çıkmaz yol tespiti
# ---------------------------------------------------------------------------
# Yöntem 1: merkez yol ROI'da büyük engel blobu
DEAD_END_MIN_AREA   = 2000
DEAD_END_ROI_LEFT   = 200
DEAD_END_ROI_RIGHT  = 440
DEAD_END_ROI_TOP    = 200
DEAD_END_ROI_BOTTOM = 320

# Yöntem 2: ileri bölgede beyaz piksel yok, yakın bölgede var
DEAD_END_FWD_WHITE_MAX  = 500    # ileri yarıda max beyaz → "şerit yok ileride"
DEAD_END_NEAR_WHITE_MIN = 2000   # yakın bölgede min beyaz → "yoldayım şu an"
...
# ---------------------------------------------------------------------------
# Çıkmaz yol dönüş parametreleri  (OVERTAKING_ parametrelerinden BAĞIMSIZ)
# ---------------------------------------------------------------------------
# Kural 3.4.6: Çıkmaz yol tabelasını/bölümünü algıla, GIRMEDEN sağa dön.
DEAD_END_TURN_SEC   = 1.8   # Sağa tank dönüşü süresi (s) — piste göre kalibre et
DEAD_END_TURN_SPEED = 45    # Dönüş sırasında motor hızı (%)
