...
**Register:** 90 distinct findings (63 confirmed software behaviors; 27 conditional risks). **Sorted by severity: Critical → High → Medium → Low. Original issue IDs are preserved.**
...
**This revision only ranks the previous findings.** No repository reinspection, new tests, or sub-agents were used. Original IDs, confirmed/conditional labels, evidence, fixes, and verification steps remain unchanged.

**Severity describes impact if the stated trigger occurs—not the likelihood of that trigger or proof of a physical incident.** A conditional hardware/configuration fault can therefore be Critical without being confirmed on the car. Disconnected sign utilities are not treated as currently commanding motors.

Within each tier, findings retain original ID order; this is not a finer ranking. Implementation dependencies may require a different repair sequence.

| Severity | Findings | Interpretation |
|---|---:|---|
| **Critical** | 9 | Potential uncontrolled or excessive motion, continued driving into a detected hazard, or failure of the stop/de-energization path under the finding's stated conditions. Treat as a stop-driving blocker wherever that path or condition applies. |
| **High** | 47 | Material steering/perception/control failure, unintended start, unsafe maneuver, or calibration defect that can lead to hazardous motion. Address before relying on the affected driving or calibration path. |
| **Medium** | 30 | Functional completion, startup, persistence, calibration-evidence, or diagnostic failure without an established immediate uncontrolled-motion outcome. Correct before operational reliance on that function. |
| **Low** | 4 | Display, instructions, or inspection usability defect with no established direct actuation effect. |

#### Critical findings at a glance

| ID | Finding | Evidence status |
|---|---|---|
| `LEGACY-008` | Raw terminal mode removes Ctrl+C stopping, while EOF spins with the last command | Confirmed |
| `LEGACY-044` | A stalled Pi capture can leave the car driving from an indefinitely old image | Confirmed |
| `LEGACY-046` | Blocking work can prevent the software stop path from running | Conditional |
| `LEGACY-054` | Parking steering bypasses the controller's forward-only and speed limits | Confirmed |
| `LEGACY-058` | Hazard handling is restricted to normal driving and misses hazards during maneuvers | Confirmed |
| `LEGACY-060` | An orange obstacle still receives a driving command when overtaking is blocked | Confirmed |
| `LEGACY-065` | Shutdown can be marked complete before any motor cleanup occurs | Conditional |
| `LEGACY-068` | Invalid deadzone settings can become full-duty motor output | Conditional |
| `LEGACY-069` | Failed motor shutdown is silently treated as complete and cannot be retried | Conditional |
...
## 3. Complete issue register — severity order

### Critical — 9 findings

Potential uncontrolled or excessive motion, continued driving into a detected hazard, or failure of the stop/de-energization path under the finding's stated conditions. Treat as a stop-driving blocker wherever that path or condition applies.

#### LEGACY-008 — Raw terminal mode removes Ctrl+C stopping, while EOF spins with the last command

**Severity:** Critical — The advertised emergency keyboard action can be ineffective, and input loss can leave the last movement command active.
...
**Severity:** Critical — A capture stall can leave motion controlled by an indefinitely stale scene without triggering the camera-error stop.
...
**Severity:** Critical — A blocked control dependency can leave the last motor command active with no timely software stop.
...
**Severity:** Critical — Parking can issue opposite-direction wheel commands and exceed its intended speed bounds, producing an unexpected powered pivot.
...
**Severity:** Critical — Moving maneuver states can continue driving despite newly detected stop hazards or passing prohibitions.
...
**Severity:** Critical — A detected obstacle can receive a continued forward-driving command precisely when overtaking is prohibited.
...
**Severity:** Critical — A shutdown diagnostic failure can prevent motor cleanup and suppress every later cleanup attempt.
...
**Severity:** Critical — With the specified invalid changed setting, a low finite motor command becomes full-duty output; the shipped setting is not affected.
...
**Severity:** Critical — Failed output disabling can be hidden and made non-retryable while the backend may retain an active command.
...
Material steering/perception/control failure, unintended start, unsafe maneuver, or calibration defect that can lead to hazardous motion. Address before relying on the affected driving or calibration path.

#### LEGACY-002 — Camera calibration, preview and driving tools use inconsistent orientation and color order

**Severity:** High — A successful calibration can be applied in the wrong orientation or color convention during driving.
...
**Severity:** High — The tool can install invalid geometry that corrupts the active lane-control input.
...
**Severity:** High — An invalid helper command can retain motion if its caller catches the error and continues; CLI final cleanup still exists.
...
**Severity:** High — Terminal restoration can delay or suppress the interactive helper's motor stop.
...
**Severity:** High — The shipped geometry omits the lower road and can substitute unrelated visual content for lane guidance.
...
**Severity:** High — Invalid perspective geometry can pass the diagnostic while the active-car startup remains permitted.
...
**Severity:** High — Blind recovery can reverse its steering direction without a new lane observation.
...
**Severity:** High — Starting or reacquiring the lane can provoke an unnecessary sharp wheel-command change.
...
**Severity:** High — If the stated forward-wheel convention matches the car, feedback steers away from the lane; physical polarity remains unverified.
...
**Severity:** High — Small input changes can produce large, abrupt inside-wheel command changes near saturation.
...
**Severity:** High — A non-signal green object can enable an unintended vehicle start.
...
**Severity:** High — Off-lane objects can incorrectly trigger or prohibit a physical overtaking maneuver.
...
**Severity:** High — Color overlap can reinterpret a parking slot as an obstacle and redirect the maneuver logic.
...
**Severity:** High — Required sign-driven behavior is unreachable through the current autonomous detector.
...
**Severity:** High — The stopping-point proxy cannot establish the required physical clearance before crossings.
...
**Severity:** High — Actual crossings can be missed, allowing the required stopping maneuver to be skipped.
...
**Severity:** High — A blank confirmation input can authorize real motor motion without explicit affirmative consent.
...
**Severity:** High — Following repeat-calibration advice can undo a working trim and impair directional control.
...
**Severity:** High — If the documented wheel convention holds, the suggested adjustment increases rather than reduces drift.
...
**Severity:** High — Saved thresholds can fit a different image/profile than the active lane detector and impair steering perception.
...
**Severity:** High — A lane-free scene can be treated as valid guidance, bypassing reported-lane-loss protection.
...
**Severity:** High — A tiny unrelated patch can become a steering target rather than a rejected observation.
...
**Severity:** High — The lane association can generate a substantially incorrect center and steering error.
...
**Severity:** High — Cached lane positions delay loss detection while the car continues using stale guidance.
...
**Severity:** High — Unexpected runtime dimensions invalidate pixel-coordinate calibration and detector geometry.
...
**Severity:** High — One acquisition can satisfy multi-frame confirmation and repeatedly update control state, including start detection.
...
**Severity:** High — Input buffering can strand a stop key as well as a start sequence until more input arrives.
...
**Severity:** High — A quit request does not prevent a subsequent motion write in the already-running iteration.
...
**Severity:** High — The readiness/start inputs can initiate driving before the actual green signal.
...
**Severity:** High — Lane loss can repeat completed overtaking progress and extend a lateral maneuver.
...
**Severity:** High — The car can return toward its original lane without confirmed obstacle clearance or lane acquisition.
...
**Severity:** High — Parking-target loss can leave the car moving indefinitely along the lane instead of stopping to recover.
...
**Severity:** High — Clock adjustments can prematurely end stop holds or extend motion phases and alter steering integration.
...
**Severity:** High — Starting the program can permit optical-green motion without an independent operator arm action.
...
**Severity:** High — Once sign recognition is connected, a restriction can expire while the car remains in the prohibited zone.
...
**Severity:** High — The car can mark a crossing serviced without reaching its stopping point, then resume toward it.
...
**Severity:** High — Normal-speed motion can resume before the bump has actually been crossed.
...
**Severity:** High — Final motor allocation erases ordinary steering corrections in bump and parking-fallback slow states.
...
**Severity:** High — A physical test can run at a higher command or different duration than the supplied options imply.
...
**Severity:** High — In a shared interpreter, tuning can change unrelated controllers' gains without restoring them.
...
**Severity:** High — The PD motion test can proceed despite its advertised stop-input channel being unavailable.
...
**Severity:** High — Cleanup can leave motion commanded for an avoidable input-thread join delay before braking.
...
**Severity:** High — A supported-looking config read can silently substitute different perspective coordinates for the active calibration.
...
**Severity:** High — The displayed tuning settings can differ from the actual processor, making active-car calibration unreliable.
...
**Severity:** High — The UI can save speed limits that conflict with one another or are exceeded by the actuator floor.
...
**Severity:** High — The preview can validate lane inputs the actual driving detector rejects and omits runtime memory behavior.
...
**Severity:** High — Shutdown can race active motor writes and resource use, compromising the no-post-stop-command boundary.
...
Functional completion, startup, persistence, calibration-evidence, or diagnostic failure without an established immediate uncontrolled-motion outcome. Correct before operational reliance on that function.

#### LEGACY-001 — Camera ownership is not protected throughout setup and cleanup

**Severity:** Medium — Can interrupt calibration and obstruct camera recovery, but no direct motion effect was established.
...
**Severity:** Medium — Version-dependent exported syntax can prevent the next configuration import and startup.
...
**Severity:** Medium — Incorrect pixel sampling misleads calibration; it does not directly command motion.
...
**Severity:** Medium — A valid parking slot can be missed because a distant red object wins candidate selection.
...
**Severity:** Medium — Incorrect stripe counting can produce an unwarranted crossing stop or approach.
...
**Severity:** Medium — Ordinary flat markings can trigger an unnecessary bump maneuver and slow mode.
...
**Severity:** Medium — Unrelated diagonals can trigger an unwarranted railway approach or stop.
...
**Severity:** Medium — Invalid measurement input produces misleading calibration advice rather than a clear rejection.
...
**Severity:** Medium — Stale menu settings can overwrite earlier calibration and make repeated tuning unreliable.
...
**Severity:** Medium — Interrupted configuration writes can prevent subsequent vehicle startup or leave a partial configuration.
...
**Severity:** Medium — Failed tests can appear successful to their launcher, undermining readiness checks.
...
**Severity:** Medium — The intended histogram smoothing does nothing, weakening robustness to visual noise.
...
**Severity:** Medium — The drive can lack its intended telemetry because waiting exhausts the recording window.
...
**Severity:** Medium — A failed export permanently loses subsequent diagnostic coverage and suppresses retry.
...
**Severity:** Medium — A healthy but slow first-frame acquisition can cause a fail-stopped startup abort.
...
**Severity:** Medium — Parking can be declared complete without establishing full-vehicle containment in the slot.
...
**Severity:** Medium — A brief detection dropout can repeat a serviced stop or maneuver instead of recognizing the same object.
...
**Severity:** Medium — The dead-end handler safely stops but never performs the required turn or resumes the course; recognition is currently disconnected.
...
**Severity:** Medium — The default GUI can prevent headless operation; backend-specific process-abort consequences remain conditional.
...
**Severity:** Medium — An interrupted constructor can leave partial resources unreleased; initial-output movement was not established.
...
**Severity:** Medium — A clamped low-speed test cannot support the trim inference it presents to the operator.
...
**Severity:** Medium — Following an invalid trim recommendation makes the motor driver reject subsequent motion.
...
**Severity:** Medium — Incomplete statistics can falsely characterize a run with substantial lane loss or oscillation as stable.
...
**Severity:** Medium — The standalone classifier assigns a known sign to unknown content; it must be corrected before autonomous integration.
...
**Severity:** Medium — The standalone classifier's candidate gates do not validate the runtime detector's candidates.
...
**Severity:** Medium — Training/inference framing differences can invalidate classifier accuracy; actual dataset framing was not available.
...
**Severity:** Medium — The writer can falsely succeed or corrupt a multiline assignment, breaking configuration persistence or startup.
...
**Severity:** Medium — Failed or partial saves can be announced as successful, leaving a different stored configuration than expected.
...
**Severity:** Medium — An increment key can instead reload settings and discard edits; mappings vary by backend.
...
**Severity:** Medium — An open video stream can block the diagnostic status endpoint and complicate server shutdown.
...
Display, instructions, or inspection usability defect with no established direct actuation effect.

#### LEGACY-006 — Threshold and bird-view switches produce exactly the same annotated image

**Severity:** Low — The promised threshold display is missing, reducing inspection usability.
...
**Severity:** Low — Incorrect menu instructions prevent the advertised calibration operation from being completed as described.
...
**Severity:** Low — Reversed plot labels misrepresent the sign of the displayed error, without changing actuation.
...
**Severity:** Low — The computed perspective preview is not displayed, limiting visual verification.
